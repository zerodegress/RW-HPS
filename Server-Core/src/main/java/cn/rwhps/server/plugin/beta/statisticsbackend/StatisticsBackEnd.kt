...
package cn.rwhps.server.plugin.beta.statisticsbackend
...
import cn.rwhps.server.plugin.beta.statisticsbackend.data.BaseData
import cn.rwhps.server.plugin.beta.statisticsbackend.net.NetConnectBack
import cn.rwhps.server.plugin.beta.statisticsbackend.net.TypeBack
...
class StatisticsBackEnd: Plugin() {
    ...
    override fun registerCoreCommands(handler: CommandHandler) {
        handler.register("starthd", "serverCommands.start") { _: Array<String>?, log: StrCons ->
            ...
            newThreadCore {
                WebData.addWebGetInstance("/api/get/dataAll", object : WebGet() {
                    override fun get(accept: AcceptWeb, send: SendWeb) {
                        ...
                        send.setHead(HttpHeaderNames.CONTENT_TYPE.toString(),"application/json; charset=UTF-8")
                        send.setData(statisticalDataSend)
                        send.send()
                    }
                })
                StartNet(StartHttp::class.java).openPort(6006)
            }

            Threads.newTimedTask(CallTimeTask.ServerUpStatistics, 0, 1, TimeUnit.MINUTES) {
                val time  = Time.concurrentSecond()-600
                statisticalData.each { t, n ->
                ...
                var RoomAllSize: Int = 0
                var RoomNoStartSize: Int = 0
                var RoomPublicListSize: Int = 0
                ...
                statisticalDataSend = BaseData(
                    ServerRunPort,ServerNetType,SystemCount,JavaCount,
                    BaseData.Companion.ServerData(ServerServerSize,ServerPlayerSize,ServerPlayerVersion,ServerIpCountry,ServerIpPlayerCountry),
                    BaseData.Companion.RelayData(RelayServerSize,RelayPlayerSize,
                        RoomAllSize,RoomNoStartSize,RoomPublicListSize,
                        RelayPlayerVersion,RelayIpCountry,RelayIpPlayerCountry)
