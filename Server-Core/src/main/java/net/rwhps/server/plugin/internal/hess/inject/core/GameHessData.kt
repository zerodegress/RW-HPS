...
internal class GameHessData: AbstractGameHessData {
    ...
    override fun getGameOverData(): GameOverData? {
        ...
        if (lastWinCount == 1) {
            ...
            val statusData = ObjectMap<String, ObjectMap<String, Int>>().apply {
                for (position in 0 until Data.configServer.maxPlayer) {
                    val player: n = n.k(position) ?: continue
                    put(player.v, PrivateClass_LinkPlayer(player).let {
    ...
    override fun getPlayerData(site: Int): AbstractLinkPlayerData {
        return PrivateClass_LinkPlayer(WaitResultUtils.waitResult { n.k(site) } ?: throw ImplementedException.PlayerImplementedException(
