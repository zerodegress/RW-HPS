...
import com.github.dr.rwserver.struct.ObjectMap.Entry;

import java.util.Iterator;
import java.util.NoSuchElementException;

/**
 * 有序或无序的物体地图。此实现使用数组存储键和值，这意味着{@link getKey（Object，boolean）get}对映射中的每个键进行比较。
 * 这比典型的散列映射实现要慢，但对于小映射可能是可以接受的，并且具有可以通过索引访问键和值的优点，这使得迭代速度更快。
 * 与{@link Seq}类似，如果ordered为false，则该类在删除元素时避免内存复制（最后一个元素移动到删除元素的位置）。
 * @author Nathan Sweet
 */
...
public class ArrayMap<K, V> implements Iterable<ObjectMap.Entry<K, V>>{
    ...
    public int put(K key, V value, int index){
        ...
        if(existingIndex != -1)
            removeIndex(existingIndex); else if(size == keys.length) //
