...
package net.rwhps.server.game.simulation.util
...
object FuckHessRes {
    fun fuck() {
        ...
        FileUtils.getFolder(Data.Plugin_GameCore_Data_Path)
            .toFile("Game-Assets.zip").zipDecoder.getZipAllBytes().eachAll { k, v ->
            var bytes = v
            when {
                k.endsWith("png") -> {
        ...
        FileUtils.getFolder(Data.Plugin_GameCore_Data_Path)
            .toFile("Game-Res.zip").zipDecoder.getZipAllBytes().eachAll { k, v ->
            var bytes = v
            when {
                k.endsWith("png") -> {
                    val image0 = ImageIO.read(DisableSyncByteArrayInputStream(v))
