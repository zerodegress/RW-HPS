...
public class Hovercraft extends HoverUnit {
    ...
    public static void load() {
        ...
        IMAGE_SHADOW = game.graphics.loadImage(R.drawable.hovercraft_shadow);
        IMAGE_WREAK = game.graphics.loadImage(R.drawable.hovercraft_dead);
    ...
    public void forceUnload() {
        ...
        while (it.hasNext()) {
            ...
            unit.x = this.x + (CommonUtils.cos(this.dir) * (-9.0f));
            unit.y = this.y + (CommonUtils.sin(this.dir) * (-9.0f));
        }
    ...
    public void update(float deltaSpeed) {
        ...
        if (!this.dead && isActive()) {
            if (this.unloading && !isOverWater() && !this.moving) {
                this.unloadDelay = CommonUtils.toZero(this.unloadDelay, deltaSpeed);
                if (this.unloadDelay == 0.0f) {
                    this.unloadDelay = 30.0f;
                    if (this.transportingUnits.size() == 0) {
                        this.unloading = false;
                    } else {
                        Unit unit = this.transportingUnits.remove(this.transportingUnits.size() - 1);
                        unit.transportedBy = null;
                        unit.x = this.x + (CommonUtils.cos(this.dir) * (-9.0f));
                        unit.y = this.y + (CommonUtils.sin(this.dir) * (-9.0f));
                        unit.xPush = 0.1f;
                        unit.dir = this.dir + 180.0f;
                        unit.dontCollideExit = this;
                        unit.dontCollideExitDelay = 45.0f;
                        if (unit instanceof OrderableUnit) {
                            OrderableUnit oUnit = (OrderableUnit) unit;
                            oUnit.clearAllWaypoints();
                            oUnit.addMoveWaypoint(this.x + (CommonUtils.cos(this.dir) * (-66.0f)), this.y + (CommonUtils.sin(this.dir) * (-66.0f)));
                        }
                        if (this.transportingUnits.size() == 0) {
                            this.unloading = false;
                        }
            ...
            this.heightCos += 4.0f * deltaSpeed;
            if (this.heightCos > 360.0f) {
                this.heightCos -= 360.0f;
            }
            if (!this.unloading) {
                this.height = CommonUtils.toValue(this.height, 3.0f + (CommonUtils.sin(this.heightCos) * 1.5f), 0.1f * deltaSpeed);
            } else {
                this.height = CommonUtils.toValue(this.height, 0.0f, 0.1f * deltaSpeed);
            }
        }
    }
    ...
    public boolean canBeTransported(Unit unit) {
        if (!this.unloading && this.transportingUnits.size() < 4 && unit != this && !(unit instanceof Hovercraft)) {
            if ((unit instanceof AirUnit) || (unit instanceof WaterUnit)) {
                return false;
            }
            if (!(unit instanceof Building) && unit.transportedBy == null) {
                return true;
            }
            return false;
        }
